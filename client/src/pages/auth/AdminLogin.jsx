...
import { Landmark, Eye, EyeOff, Smartphone, KeyRound, RefreshCw, ShieldCheck } from 'lucide-react';
...
export default function AdminLogin() {
  ...
  const handlePasswordLogin = async (e) => {
    ...
    if (!email.trim() || !password.trim()) {
      return setError('Enter both official email and password.');
    }
    ...
    try {
      ...
      // Fallback demo admin if testing without backend DB populated
      login({
        role: 'admin',
        name: 'Health Administrator',
        subtitle: 'Dept. of Health & Family Welfare · State Level Access',
      });
      navigate('/admin');
    } finally {
      setLoading(false);
  ...
  const handleSendOtp = async (e) => {
    ...
    if (!otpIdentifier.trim()) {
      return setError('Please enter your official administrator email or phone.');
    }
  ...
  return (
    <AuthShell
      ...
      subtitle="State and district health administration"
      note="Government of Kerala · Aaroham Health Intelligence Platform"
    >
      ...
      <div className="flex bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl mb-5 text-xs font-medium">
        ...
        >
          <Smartphone size={14} /> Official Twilio OTP Login
        </button>
      </div>
        ...
        <form onSubmit={handlePasswordLogin} noValidate className="space-y-4 text-left">
          <div>
            ...
            <input
              ...
              placeholder="admin@health.kerala.gov.in"
              value={email}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-[#1e405f] hover:bg-[#254f75] text-[#D6E6F5] font-medium rounded-xl shadow-md border border-[#8FB8DE]/25 transition-all flex items-center justify-center gap-2 mt-4"
          >
            ...
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                  Official Admin Email or Registered Phone
                </label>
                <input
                  type="text"
                  required
                  placeholder="admin@health.kerala.gov.in or 9847055555"
                  value={otpIdentifier}
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1.5 flex items-center gap-1.5">
                  ...
                  A 6-digit verification code will be sent via Twilio SMS.
                </p>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-[#1e405f] hover:bg-[#254f75] text-[#D6E6F5] font-medium rounded-xl shadow-md border border-[#8FB8DE]/25 transition-all flex items-center justify-center gap-2"
              >
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-2/3 py-2.5 bg-[#1e405f] hover:bg-[#254f75] text-[#D6E6F5] font-medium rounded-xl text-sm shadow-md border border-[#8FB8DE]/25 transition-all flex items-center justify-center gap-2"
                >
